...
//$json = file_get_contents('test1.json');
 ...
 $json = json_decode($json,true)['DirList'];
 ...
 function scan($arr,$path = '/'){//扫描方法
   if(isset($arr['Name'])){
       ...
       //$dir[$dpath]['files'] = $arr['files'];//储存文件
        $dir[$dpath]['ali'] = toAli($v['FileList']);//储存秒传码
        
      if(!empty($v['DirList'])){//若有子目录
         $dir = $dir+scan($arr['DirList'],$dpath);//扫描子目录
       }
     return $dir;
   }else{
       ...
       if($arr !== NULL){foreach($arr as $k => $v){
         ...
         $dir[$dpath]['ali'] = toAli($v['FileList']);//储存秒传码
        if(!empty($v['DirList'])){//若有子目录
           $dir = $dir+scan($v['DirList'],$dpath);//扫描子目录
         }
       }}
       return $dir;
   }
